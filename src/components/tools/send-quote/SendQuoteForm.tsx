...
export default function SendQuoteForm() {
  ...
  const handleItemChange = (
    ...
    const totalMandays = newQuoteItems.reduce(
      (sum, item) => sum + (item.mandays || 0),
      0
    );
    const totalOneTimeCostMilVND = newQuoteItems.reduce(
      (sum, item) => sum + (item.costMilVND || 0),
      0
    );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        'https://n8n.rockship.co/webhook/send-quote',
        {
          method: 'POST',
          headers: {
            'Content-Type':
  ...
  const handleDeleteItem = (index: number) => {
    ...
    const newQuoteItems = quoteData.quotationItems.filter(
      (_, i) => i !== index
    );
              ...
              <div className='flex items-start justify-between mb-8'>
                <div>
                  ...
                  <span className='text-muted-foreground'>
                    OXLEY BIZHUB, 73 UBI ROAD 1, #08-54, Postal 408733
                  </span>
                </div>
                <div>
                ...
                <table className='w-full border-collapse'>
                  ...
                  <tbody>
                    ...
                    <tr className='bg-foreground text-background font-semibold'>
                      ...
                      <td className='border p-3'>
                        {quoteData.totalOneTimeCostMilVND}
                      </td>
                    </tr>
                  </tbody>
                </table>
